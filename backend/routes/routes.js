...
router.post('/', async (req,res)=>{
    try{
        ...
        console.log(bookName,bookId,pubName,dop,noPages,yop,cost)
        if (!bookName||!bookId||!pubName||!dop||!noPages||!yop||!cost){
            return res.status(500).json({msg:"All Fields are Required"});
        }
        temp=await Book.findOne({bookId:Number(bookId)});
        
        if(temp){
            console.log(temp);
            return res.status(200).json({msg:"Id Already Exists "});
        }
...
router.get('/:bookId',async (req,res)=>{
    try{
        ...
        const user =await Book.findOne({bookId:bookId});
        console.log(user);
        if(user){
